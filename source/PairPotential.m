function w = PairPotential(xi, energy_index, sym_flag)

%% PAIRPOTENTIAL computes the sum of repulsion energies corresponding to 
% given distances
%
% Input
...
%
% Output
% w       sum of repulsion energies corresponding to the entries of xi
%         (i.e. scalar output)
%
% See also DPAIRPOTENTIAL
...
if energy_index == 0
    ...
    if sym_flag == 0
        w   = sum(  xi .* (log(xi/pi)-1.0) ) + pi*length(xi) ;
    else
        w   = sum(  xi .* log(xi/pi) - xi + (pi-xi) .* (log(1.0 - xi/pi)-1.0) ) + 2*pi*(length(xi));
    end
elseif energy_index > 0
    %% hyperbolic function as force (no cutoff)
    w   = sum( 1./ (xi.^energy_index) );
    if sym_flag == 1
        w   = w + sum( 1./ ((pi-xi).^energy_index) );
    end
else
    error('PairPotential not yet implemented for energy_index < 0')
end
