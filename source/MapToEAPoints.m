function [X_sym_rot, X_eq, D_sym_rot, D_eq ] = MapToEAPoints( X )
...
n           = 2*size(X,2);
X_eq        = eq_point_set(2,n);
X_sym_rot   = RotateToPoles( X, X_eq );
D_sym_rot   = GeodesicDistance(X_sym_rot);
D_eq        = GeodesicDistance(X_eq);
for i=1:n
    D_sym_rot(i,:)  = sort(D_sym_rot(i,:), 'ascend');
    D_eq(i,:)       = sort(D_eq(i,:), 'ascend');
end
